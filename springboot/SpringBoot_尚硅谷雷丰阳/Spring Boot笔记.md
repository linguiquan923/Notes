...
![image-20201209143829903](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20201209143829903.png)
...
```java
http://localhost:8080/asserts/js/Chart.min.js //如果无法访问直接重启idea
```
...
public class ThymeleafProperties {
	...
	public static final String DEFAULT_SUFFIX = ".html";
  	//只要我们把html页面放在classpath:/templates/，自动帮我们渲染
```
...
​	th：任意html属性；来替换原生属性的值，接管了html的所有属性
...
### 2、扩展SpringMVC，创建一个resources/springmvc.xml文件
...
**==编写一个配置类（@Configuration），是WebMvcConfigurerAdapter类型；不能标注@EnableWebMvc、不能标注@EnableWebMvc==**;

既保留了所有的自动配置，也能用我们扩展的配置；

在主程序的同一个文件夹新建config/MyMvcConfig
...
2）、我们使用@EnableWebMvc会自动导入一个 WebMvcConfigurationSupport
...
3）、如果导入了 WebMvcConfigurationSupport，那么自动配置就会失效
...
就算我们以后给自己的项目添加上了访问路径，那么Thymeleaf也会自动修改的

```properties
server.servlet.context-path=/crud
```

![image-20201209154400471](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20201209154400471.png)
...
![image-20201209155728282](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20201209155728282.png)

![image-20201209155507133](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20201209155507133.png
...
在application.properties中设置国际化的basename
...
这个时候我们配置出来的会乱码，需要在file encoding里面修改
...
@PostMapping(value = "/user/login")
    public String login(@RequestParam("username") String username,@RequestParam("password") String password, Map<String,Object> maps){
...
给我们的登录的时候controller层添加一个session
...
访问数据库近需要配置的东西（需要配置serverTimezone）
...
![image-20201210102408339](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20201210102408339.png)
...
每一个mapper文件都得加@Mapper注解太过于麻烦，所以我们可以在主程序那个位置配置一个@MapperScan();
...
mybtis/mapper/DepartmentMapper.xml  //配置映射文件

mybtis/mybatis-config.xml //mybatis核心文件

在applictaion.yaml里面配置
...
创建springboot项目引入JPA JDBC MYSQL 模块
...
```java
//继承JpaRepository，这次传入的User是指数据库对应的实体类，Integer指主键对应的数据类型
...
@ConfigurationPropertie //结合相关xxxProperties类来绑定相关的配置，我们在properties中可以配置的东西
...
如果要配置starter里面的东西，我们可以在application.properties文件里面配置
